...
1. `cluster01` 우선 subset 실험 설계를 구체화한다
2. `cluster02` 경량 subset 재검증 범위를 정리한다
3. 이후 전체 공통 스테이션 `station-hour` 상위 오류를 확장 분석한다
4. 상위 오류 스테이션이 특정 군집에 몰리는지 확인한다
5. `CatBoost`는 축소 실험 또는 경량 파라미터 조건으로 별도 검토한다
6. 수요 예측값을 `재배치 판단 지표`와 `예상 잔여 자전거 수`로 변환하는 후처리 규칙 초안을 작성한다
7. 운영자 화면 / 일반 사용자 화면을 분리한 API·화면 단위 정의로 넘어간다
8. 예측 파트 결과를 발표 문서에 반영한다

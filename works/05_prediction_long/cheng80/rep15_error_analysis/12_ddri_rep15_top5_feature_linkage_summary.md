...
즉 현재는 `오류 우선순위 -> top2(상위 2개) 시간대 패턴 -> top5(상위 5개) 확장 패턴 -> 피처 연결 및 축소 피처 조합(subset) 우선순위` 흐름을 이 문서 하나로 수렴해서 읽는 것이 맞다.
...
- 우선 피처:
  ...
  - `commute_evening_flag`(퇴근 시간대 여부)
...
- 우선 피처:
  ...
  - `distance_river_boundary_m`(하천 경계까지 거리)
  - `bus_stop_count_300m`(300m 안 버스정류장 수)
...
- 최종 해석은 top2(상위 2개)가 아니라 top5(상위 5개) 기준과 축소 피처 조합(subset) 우선순위까지 포함한 이 문서를 기준으로 읽는다

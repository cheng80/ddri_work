...
작성일: 2026-03-15  
목적: `cluster01` 아침 도착 업무 집중형의 다음 subset 실험을 바로 실행 가능한 수준으로 고정한다.
...
1. `cluster01`에 대해 `LightGBM_Poisson` 우세가 subset 조건에서도 유지되는지 확인
2. 모든 후보 피처를 다 쓰는 방식이 아니라, `핵심 피처 subset`만으로도 성능이 유지되거나 개선되는지 확인
...
다음 subset 실험은 반드시 위 3개와 같은 방식으로 비교한다.
...
- 최우선 선택 지표:
  - `test RMSE`
- 비교 대상 모델:
  - 기본은 `LightGBM_Poisson`
  - 필요 시 대조군으로 `LightGBM_RMSE` 1개 유지
...
4. subset B 실행
5. subset C 실행
6. 필요하면 각 subset에 대해 `LightGBM_RMSE` 대조군 1회 실행
7. 결과를 하나의 비교표로 정리

        ...
        rows.append(
            {
                ...
                "dtype_train": str(train_df[col].dtype) if col in train_df.columns else None,
                "dtype_test": str(test_df[col].dtype) if col in test_df.columns else None,
            }
        )
    rows.extend(
        [
...
def main() -> None:
    ...
    log("[2/4] Train 2023 기준 seasonal mean 생성 + bike_change 타깃 생성")
    ...
    feature_summary.to_csv(SUMMARY_OUT, index=False, encoding="utf-8-sig")
    ...
    meta = {
        ...
        "feature_summary_output": str(SUMMARY_OUT),

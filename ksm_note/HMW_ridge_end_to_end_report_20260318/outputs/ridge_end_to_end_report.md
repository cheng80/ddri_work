...
## 6. 데이터 분할과 모델링
### 6-1. 가중치 -> 선형회귀식 -> Ridge 적용 흐름
- 먼저 월별 반복패턴 분석 결과를 이용해 각 학습 행의 `sample_weight`를 계산했습니다.
- 다음으로 선형회귀식 형태(`y_hat = b0 + b1x1 + ... + bkxk`)로 타깃을 설명했습니다.
- 마지막으로 같은 선형식에 L2 규제를 더한 `Ridge`를 사용해, 다중공선성 상황에서도 계수가 과도하게 흔들리지 않게 학습했습니다.
- 즉, 이 보고서의 모델은 `가중치가 적용된 선형회귀식`을 기반으로 하고, 이를 Ridge 목적함수로 최적화한 구조입니다.

```text
1) sample_weight 계산
2) 선형회귀식 설정: y_hat = b0 + b1x1 + ... + bkxk
3) Ridge 학습: min Σ w_i (y_i - y_hat_i)^2 + α Σ b_j^2
```

![weighted ridge flow](weighted_ridge_formula_flow.png)

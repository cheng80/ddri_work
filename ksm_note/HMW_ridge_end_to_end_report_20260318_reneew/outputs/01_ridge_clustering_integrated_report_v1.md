...
- 본 문서는 강남구 따릉이 대여소를 `공간 역할` 기준으로 군집화하고, 그 결과를 바탕으로 `대여소-시간 단위(station-hour)` 수요 예측 분석까지 연결한 1차 통합본이다.
- 군집화 결과가 예측 분석으로 어떻게 이어지는지 한 문서 안에서 확인할 수 있도록 구성했다.

## 핵심 결과

- 군집화는 대여소를 `업무/상업형`, `주거형`, `생활·상권 혼합형`, `외곽형` 같은 공간 역할 기준으로 분류하기 위한 단계였다.
- 예측 분석에서는 자전거 수요 변화량(`bike_change_raw`)을 설명 가능한 방식으로 예측하기 위해 공선성 정리, 누수 점검, 시계열 패턴 진단, `학습 가중치(sample_weight)` 적용을 거친 Ridge 회귀를 구성했다.
- Ridge는 `train R2=0.726576`, `valid R2=0.721155`, `test R2=0.727374`로 분할(`split`) 간 편차가 크지 않은 안정적인 기준 모델로 정리됐다.
...
- 단순한 대여량 규모만으로는 각 대여소의 역할을 설명하기 어렵기 때문에, 먼저 `대여소 단위(station-level)` 공간 역할 분류가 필요했다.
...
- 이후 날씨·생활인구·고저차 등 환경 피처를 선별해 `대여소-일 단위(station-day)` 또는 `대여소-시간 단위(station-hour)` 수요 예측으로 연결
...
- 전체 원천 이용 이력 행 수: `2,896,795`
- 최종 분석 사용 행 수: `2,663,938`
- 전체 제외 행 수: `232,857 (8.04%)`
...
<div class="callout compact">군집 결과를 예측 피처로 활용하려면, 군집이 실제로 분리되고 서로 다른 특성을 가진다는 근거가 함께 제시되어야 한다.</div>
...
- 공선성 정리와 중복 변수 제거
- 누수 및 시간순 분할 검증
- 월별 반복 패턴 진단과 `학습 가중치(sample_weight)` 적용
...
- 학습 데이터는 월별 반복 패턴 보정을 위해 `학습 가중치(sample_weight)`가 반영된 구조를 사용
- 원본 정제 데이터(`canonical`) 컬럼 수: **26개**
- 최종 입력 변수 수: **16개**
- 최종 예측 타깃: 자전거 수요 변화량 (`bike_change_raw`)

<div class="chart-block tight">
  <img class="chart-image" src="../../HMW_ridge_end_to_end_report_20260318/outputs/feature_count_summary.png" alt="변수 개수 요약" />
</div>
...
| 시간대 (`hour`) | 0~23시 시간 구간 | 대여소 군집 번호 (`cluster`) | 군집(`cluster`) 결과 라벨 |
...
<div class="chart-block compact">
  <img class="chart-image" src="../../HMW_ridge_end_to_end_report_20260318/outputs/high_correlation_pairs_bar.png" alt="높은 상관계수 변수 쌍" />
</div>
...
| 장기 시차 변수 (`bike_change_lag_24 / bike_change_lag_168`) | 다중 시차 변수 과다 | 대표 시차(`lag`)만 남기고 제거 |
...
<div class="chart-block">
  <img class="chart-image" src="../../HMW_ridge_end_to_end_report_20260318/outputs/current_feature_correlation_heatmap.png" alt="최종 입력 변수 상관계수 히트맵" />
</div>

<div class="page-break"></div>

# 6. 누수 점검과 학습 가중치(sample_weight) 설계
...
<div class="chart-block">
  <img class="chart-image" src="../../lightgbm_reason/outputs/target_like_feature_correlation.png" alt="타깃 유사 피처 상관계수" />
</div>

## 왜 학습 가중치(sample_weight)가 필요했는가
...
- 대여 건수(`rental_count`)의 `2023-05`와 `2023-06`은 `상관계수(corr)=0.9947`, `NRMSE=0.1074`로 매우 유사했다.
- 이는 같은 패턴을 여러 달이 거의 중복해서 보여준다는 뜻이다.

<div class="chart-block small">
  <img class="chart-image" style="width: calc(100% + 24mm); max-width: none; margin-left: -12mm; max-height: none;" src="../../HMW_ridge_end_to_end_report_20260318/outputs/adjacent_month_similarity_heatmap.png" alt="인접 월 유사도 히트맵" />
</div>

<div class="page-break"></div>

<div class="chart-block">
  <img class="chart-image" src="../../HMW_ridge_end_to_end_report_20260318/outputs/overall_month_weights.png" alt="월별 가중치" />
</div>

<div class="chart-block">
  <img class="chart-image" src="../../HMW_ridge_end_to_end_report_20260318/outputs/hourly_profile_compare_rental_count.png" alt="시간대별 패턴 비교" />
</div>
...
- 모델: `Ridge(alpha=2.0)`
- 전처리: 중앙값 대치(`median imputation`) + 표준화(`standardization`)
- 학습 시 `학습 가중치(sample_weight)` 적용
- 평가 지표: `RMSE`, `MAE`, `R2`
...
<div class="chart-block compact">
  <img class="chart-image" style="width: 80%; max-width: none; margin-left: 0; max-height: none;" src="../../HMW_ridge_end_to_end_report_20260318/outputs/split_row_counts.png" alt="분할별 행 수" />
</div>
...
- `train R2=0.726576`, `valid R2=0.721155`, `test R2=0.727374`로 분할(`split`) 간 차이가 크지 않았다.
...
- 따라서 LightGBM의 높은 점수는 `강한 과거 이력 피처(feature) + 반복되는 시계열 패턴`의 영향이 크다고 해석했다.
...
<div class="chart-block">
  <img class="chart-image" src="../../lightgbm_reason/outputs/repeated_pattern_similarity_counts.png" alt="반복 패턴 수" />
</div>

## 클러스터별 Ridge 결과

- 검증 기준 최고 클러스터와 테스트 기준 최고 클러스터를 별도로 비교해 군집별 난이도 차이를 확인했다.
- `군집 1(cluster 1)`은 상대적으로 성능이 낮아 군집 특화 피처 보강이 필요한 구간으로 볼 수 있다.

<div class="chart-block">
  <img class="chart-image" src="../../HMW_cluster_ridge_regression_20260318/outputs/cluster_r2_by_split.png" alt="군집별 Ridge R²" />
</div>

<div class="chart-block">
  <img class="chart-image" src="../../HMW_cluster_ridge_regression_20260318/outputs/cluster_rmse_by_split.png" alt="군집별 Ridge RMSE" />
</div>
